 ...
 */
int git_mwindow_contains(git_mwindow *win, git_object_size_t offset)
{
	git_object_size_t win_off = win->offset;
	return win_off <= offset
		&& offset <= (git_object_size_t)(win_off + win->window_map.len);
}
	...
	git_object_size_t offset)
{
	...
	len = size - w->offset;
	if (len > (git_object_size_t)git_mwindow__window_size)
		len = (git_object_size_t)git_mwindow__window_size;
