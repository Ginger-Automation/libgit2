...
struct git_indexer {
	...
	unsigned int mode;
	git_object_size_t off;
	git_object_size_t entry_start;
...
static int hash_header(git_hash_ctx *ctx, git_object_size_t len, git_object_t type)
{
	char buffer[64];
	size_t hdrlen;
	int error;
...
static int crc_object(uint32_t *crc_out, git_mwindow_file *mwf, git_object_size_t start, git_object_size_t size)
{
	void *ptr;
	uint32_t crc;
...
static int store_object(git_indexer *idx)
{
	...
	git_oid oid;
	struct entry *entry;
	git_object_size_t entry_size;
	struct git_pack_entry *pentry;
	git_object_size_t entry_start = idx->entry_start;
...
static int save_entry(git_indexer *idx, struct entry *entry, struct git_pack_entry *pentry, git_object_size_t entry_start)
{
	int i;
...
static int hash_and_save(git_indexer *idx, git_rawobj *obj, git_object_size_t entry_start)
{
	git_oid oid;
	size_t entry_size;
	struct entry *entry;
...
static int write_at(git_indexer *idx, const void *data, git_object_size_t offset, size_t size)
{
	git_file fd = idx->pack->mwf.fd;
	size_t mmap_alignment;
	size_t page_offset;
	git_object_size_t page_start;
...
{
	git_object_size_t new_size;
	size_t mmap_alignment;
	size_t page_offset;
	git_object_size_t page_start;
	git_object_size_t current_size = idx->pack->mwf.size;
...
static int read_stream_object(git_indexer *idx, git_indexer_progress *stats)
{
	...
	git_object_size_t entry_start = idx->off;
	size_t entry_size;
	git_object_t type;
	git_mwindow *w = NULL;
	int error;
